...
int main(int argc, char **argv) {  
  ...
  double psi_one_for_nodes_even = atof(argv[3]);
  double psi_zero_for_nodes_odd = atof(argv[4]);  
  ...
  for (int number_of_nodes = 0; number_of_nodes < 4; number_of_nodes ++) {    
    if ((number_of_nodes % 2) == 0) {
      psi_0 = 0.0;
      psi_1 = psi_one_for_nodes_even;
    } else {
      psi_0 = psi_zero_for_nodes_odd;
      psi_1 = get_psi_1_for_nodes_odd(psi_0);
    }
